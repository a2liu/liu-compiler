...
impl<'writer, W: Write> Renderer<'writer, W> {
    ...
    /// Location focus.
    fn snippet_locus(&mut self, locus: &Locus) -> FmtResult {
        write!(
            self,
    ...
    fn outer_gutter(&mut self, outer_padding: usize) -> FmtResult {
        write!(self, "{space: >width$} ", space = "", width = outer_padding)?;
        Ok(())
    }

    /// The outer gutter of a source line, with line number.
    fn outer_gutter_number(&mut self, line_number: usize, outer_padding: usize) -> FmtResult {
        write!(
            self,
            "{line_number: >width$}",
    ...
    fn border_left(&mut self) -> FmtResult {
        return write!(self, "{}", '|');
    }

    /// The broken left-hand border of a source line.
    fn border_left_break(&mut self) -> FmtResult {
        return write!(self, "{}", '·');
    }
    ...
    ///  │
    /// ```
    fn label_multi_left(&mut self, underline: bool) -> FmtResult {
        match underline {
            false => write!(self, " ")?,
            true => write!(self, "{}", '-')?,
        }
    ...
    ///  ╭
    /// ```
    fn label_multi_top_left(&mut self) -> FmtResult {
        return write!(self, " {}", '╭');
    }
    ...
    ///  ╰
    /// ```
    fn label_multi_bottom_left(&mut self) -> FmtResult {
        return write!(self, " {}", '╰');
    }
    ...
    /// ─────────────^
    /// ```
    fn label_multi_top_caret(&mut self, source: &str, start: usize) -> FmtResult {
    ...
    /// ─────────────^ expected `Int` but found `String`
    /// ```
    fn label_multi_bottom_caret(&mut self, source: &str, start: usize, message: &str) -> FmtResult {
        for (metrics, _) in self
            .char_metrics(source.char_indices())
    ...
    fn inner_gutter_column(&mut self, underline: Option<Underline>) -> FmtResult {
        match underline {
    ...
    fn inner_gutter_space(&mut self) -> FmtResult {
        write!(self, "  ")?;
        Ok(())
    }
...
fn line_index(files: &FileDb, id: u32, idx: usize) -> core::result::Result<usize, FmtError> {
    files.line_index(id, idx).ok_or(FmtError)
}

fn line_range(files: &FileDb, id: u32, idx: usize) -> core::result::Result<Range<usize>, FmtError> {
    files.line_range(id, idx).ok_or(FmtError)
}

fn line_number(files: &FileDb, id: u32, idx: usize) -> core::result::Result<usize, FmtError> {
    files.line_number(id, idx).ok_or(FmtError)
}

fn location(files: &FileDb, id: u32, idx: usize) -> core::result::Result<Location, FmtError> {
    files.location(id, idx).ok_or(FmtError)
}

fn name(files: &FileDb, id: u32) -> core::result::Result<&str, FmtError> {
    files.name(id).ok_or(FmtError)
}

fn source(files: &FileDb, id: u32) -> core::result::Result<&str, FmtError> {
    files.source(id).ok_or(FmtError)
}
